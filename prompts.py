...
2. **JSON Format**:
   ...
   - Each dictionary must have exactly two keys: `method` (uppercase HTTP method, e.g., GET, POST) and `path` (starting with `/`, character case preserved exactly as in the `rails routes` output).
   - Example:
     [
...
File Content:
Return only the JSON array. No prose, no explanations, no markdown fences.
...
2. **JSON Format**:
   - Output must be a JSON array of dictionaries.
   - Each dictionary must have exactly two keys: `method` (uppercase HTTP method, e.g., GET, POST) and `path` (starting with `/`, character case preserved exactly as written in the source code).
   - Example:
     [
...
express_endpoint_extractor_prompt = """
            ...
            4. **Validation**:
               ...
               - Preserve the character case of paths exactly as written in the source.
...
django_endpoint_extractor_prompt = """You are a Django routing expert. Analyze the provided Django urls.py file and return a comprehensive and VALID JSON array of all possible endpoints it defines. Ensure the following rules are strictly followed:
        ...
        4. Validation:
            - Ensure every path starts with /.
            - Ensure methods are uppercase (GET, POST, etc.).
            - Preserve the character case of paths exactly as written in the source, and include trailing slashes unless explicitly configured otherwise.
...
flask_endpoint_extractor_prompt = """
        ...
        4. Validation:
            - Ensure every path starts with /.
            - Ensure methods are uppercase (GET, POST, etc.).
            - Preserve the character case of paths exactly as written in the source, and reflect the defined structure.
...
generic_endpoint_extractor_prompt = """You are a web framework routing expert. Analyze the provided source file and return a comprehensive and VALID JSON array of all HTTP API endpoints it defines. The file may come from any web framework (Spring, Laravel, NestJS, or another REST framework). Ensure the following rules are strictly followed:

            1. **Consistency**:
               - Every output must follow the same format and include all valid endpoints.
               - Missing any valid endpoint or including invalid ones is unacceptable.
               - Do not invent endpoints that are not defined in the file.

            2. **JSON Format**:
               - Output must be a JSON array of dictionaries and nothing else. No prose, no markdown fences.
               - Each dictionary should have exactly two keys: "method" and "path".
               - Example:
                 [
                   {{"method": "GET", "path": "/example"}},
            ...
            3. **Rules for Extraction**:
               - Include every HTTP method the file registers (GET, POST, PUT, PATCH, DELETE, OPTIONS, HEAD).
               - Prepend any class-level, controller-level, or group-level path prefix to each endpoint path.
               - Preserve path parameters exactly as written in the source (e.g. /users/{{id}}, /users/:id).
               - Ensure every path starts with `/`.
...
generic_endpoint_extractor_system_prompt = "You are an expert web developer and API routing specialist."
